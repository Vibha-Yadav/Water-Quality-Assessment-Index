...
const Temperature = () => {
  ...
  const parseCSV = (csv) => {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (row.length >= Math.max(timestampIndex, temperatureIndex)) {
        parsedData.push({
          timestamp: row[timestampIndex],
          temperature: parseFloat(row[temperatureIndex]) // Assuming Temperature values are numeric
        });
      }
    }
  ...
  const filterDataByInterval = (interval) => {
    ...
    data.forEach((entry) => {
      ...
      groupingMap[key].sum += entry.temperature;
    ...
    for (const key in groupingMap) {
      if (groupingMap.hasOwnProperty(key)) {
        ...
        filteredData.push({ timestamp: key, temperature: average });
      }
    }

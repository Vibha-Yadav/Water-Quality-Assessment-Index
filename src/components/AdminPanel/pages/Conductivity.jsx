...
const Conductivity = () => {
  ...
  const parseCSV = (csv) => {
    ...
    const conductivityIndex = headerRow.indexOf('Conductivity'); // Assuming "Conductivity" is the column name for conductivity values
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (row.length >= Math.max(timestampIndex, conductivityIndex)) {
        parsedData.push({
          timestamp: row[timestampIndex],
          conductivity: parseFloat(row[conductivityIndex]) // Assuming Conductivity values are numeric
        });
      }
    }
  ...
  const filterDataByInterval = (interval) => {
    ...
    data.forEach((entry) => {
      ...
      groupingMap[key].sum += entry.conductivity;
    ...
    for (const key in groupingMap) {
      if (groupingMap.hasOwnProperty(key)) {
        ...
        filteredData.push({ timestamp: key, conductivity: average });
      }
    }
  ...
  return (
    <div>
      ...
      <YAxis label={{ value: '(μS/cm)', angle: -90, position: 'insideLeft' }} />
        <CartesianGrid stroke="#eee" strokeDasharray="5 5" />
        <Tooltip />
        <Legend />
        <Line type="monotone" dataKey="conductivity" stroke="#82ca9d" /> {/* Change dataKey to conductivity */}
      </LineChart>
      </div>
    </div>
  );
